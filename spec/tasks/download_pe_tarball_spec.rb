...
describe "download_pe_tarball.rb" do
  ...
  describe "#valid_url?" do
    context "when the url is valid" do
      it "returns true" do
        expect(valid_url?(TEST_VALID_URL)).to eq(true)
      end
    end
    ...
    context "when the url is not valid" do
      it "reports the error and returns false" do
        ...
        expect(valid_url?(TEST_INVALID_URL)).to eq(false)
      end
    end
  end
  ...
  describe "#valid_extension?" do
    context "when the extension is valid" do
      it "returns true" do
        expect(valid_extension?(TEST_VALID_URL)).to eq(true)
      end
    end
    ...
    context "when the extension is not valid" do
      it "reports the error and returns false" do
        ...
        message1 = "Invalid extension: #{extension} for URL: #{TEST_INVALID_URL}."
        message2 = "Extension must be .gz"

        allow(self).to receive(:puts)
        expect(self).to receive(:puts).with(message1)
        expect(self).to receive(:puts).with(message2)
        expect(valid_extension?(TEST_INVALID_URL)).to eq(false)
      end
    end
  end
  ...
  describe "#valid_input?" do
    context "when the input is valid" do
      before do
        instance_variable_set(:@url, TEST_VALID_URL)
        instance_variable_set(:@destination, TEST_DEST)
      end

      it "returns true" do
        expect(self).to receive(:valid_url?).with(TEST_VALID_URL).and_return(true)
        expect(self).to receive(:valid_extension?).with(TEST_VALID_URL).and_return(true)
    ...
    context "when the input is not valid" do
      ...
      before do
        instance_variable_set(:@url, TEST_INVALID_URL)
      end

      it "returns false" do
        expect(self).to receive(:valid_url?).with(TEST_INVALID_URL).and_return(false)
        expect(valid_input?).to eq(false)
      end
    end
  end
